const API_URL = "https://script.google.com/macros/s/AKfycbybTQ1ybF0Z39xdUVyj5-Ab6U_7aNhY5t6zr0g4FHJ_cDYQ9MuDtbKLMY1MiLujBGSBbQ/exec"; // REPLACE THIS
        ...
        const syncItem = async (storeName, actionName, idField, deleteOnSuccess = false) => {
            ...
            for (const item of items) {
                if (item.syncStatus === "Pending" || deleteOnSuccess) { 
                    try {
                        // ✅ FIX: Hapus mode: 'no-cors' karena membuat response gagal terbaca
...
window.openShiftReport = async function() {
    ...
    shiftOrders.forEach(o => {
        ...
        // KARENA KITA SUDAH PAKAI JSON DI KOLOM S, SEMUA ORDER PASTI PUNYA o.items
        if (o.items && Array.isArray(o.items) && o.items.length > 0) {
            o.items.forEach(i => { 
                let lineTotal = i.qty * (Number(i.price) || Number(i.originalPrice) || 0);
                if(i.location && String(i.location).toLowerCase().includes('laundry')) orderOmsetL += lineTotal;
            ...
            // Jaga-jaga jika ada transaksi lama (sebelum update JSON ini)
